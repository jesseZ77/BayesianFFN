...
class BayesianRegressionWrapper(nn.Module):
    ...
    def set_params(self, w): # verified
...
class RegressionSampler:
    ...
    def compute_gradient_update(self, X, y, w, tausq): # verified
        self.model.set_params(deepcopy(w))
        self.model.tausq = tausq

        # y_num = y.numel()
        y_pred = self.model(X)
        # sse = y_num * nn.MSELoss()(y_pred, y)
    ...
    @torch.no_grad()
    def log_prop_diff(self, w_p, w_p_bar, w_k, w_k_bar): # verified
